...
extern "C" __declspec(dllexport) bool __cdecl CNC_Save_Load(bool save, const char *file_path_and_name, const char *game_type)
{
	...
	if (save) {
		...
		//FluffyQuack: This makes the 1-gap building mod work with savegames created without the mod running
		//DLLExportClass::Recalculate_Placement_Distances(); //Commented out as the game tries to load the DLL a savefile was saved with, so this ends up doing nothing
		...
		for (int x = 0; x < map_cell_width; x++) {
			...
			if ((base && base->House->Class->House == PlayerPtr->Class->House) || (Map[cell].Owner == PlayerPtr->Class->House)) {
				...
				for (FacingType facing = FACING_N; facing < FACING_COUNT; facing++) {
					...
					//FluffyQuack: Repeat of above to let building placements be allowed by one extra tile
					//if (GameToPlay != GAME_NORMAL) {
					...
					//} // Kerekupai: end of FluffyQuack mod
				}
			}
		}
	}
